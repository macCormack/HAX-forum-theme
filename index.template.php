...
//	</style>';
  ...
  <link rel="stylesheet" href="./themes/default/dist/assets/css/app.css">
</head>
...
function template_body_above()
{
  ...
  <div class="page-scroll">
	...
	// Skip nav link.
	echo '
	<header id="top_section" class="header">
	...
	echo '
		...
		<div id="header" class="forum-title-container', !empty($settings['header_layout']) ? ($settings['header_layout'] == 1 ? ' centerheader' : ' rightheader') : '', '"', empty($context['minmax_preferences']['upshrink']) ? '' : ' style="display: none;" aria-hidden="true"', '>
			<h1 id="forumtitle">
				<a class="forumlink" href="', $scripturl, '"><img class="logo" src="./themes/default/images/logo.png"></a>
			</h1>';
	...
	echo '
		<nav id="menu_nav" class="navigation" role="navigation">
	...
	echo '
		<div id="main_content_section" class="main-content"><a id="skipnav"></a>';
}
...
function template_th_search_bar()
{
	...
	echo '
			<form id="search_form" class="top-search-form" action="', $scripturl, '?action=search;sa=results" method="post" accept-charset="UTF-8">
	...
	echo '
				<input type="submit" name="search;sa=results" value="', $txt['search'], '" class="button_submit inversed', (!empty($modSettings['search_dropdown'])) ? ' with_select' : '', '" />
...
function template_html_below()
{
	...
	echo '
  <script src="themes/default/dist/assets/js/app.js"></script>
</body>
</html>';
}
...
function theme_linktree($default = 'linktree')
{
	...
	foreach ($context[$default] as $pos => $tree)
	{
		...
		echo $settings['linktree_link'] && isset($tree['url']) ? '<a href="' . $tree['url'] . '">' . ($pos == 0 ? '<i class="fa fa-home fa-lg"></i>' : $tree['name']) . '</a> <i class="fa fa-chevron-right" aria-hidden="true"></i>' : $tree['name'];
...
function template_menu()
{
	...
	// WAI-ARIA a11y tweaks have been applied here.
	echo '
					<ul id="main_menu" class="nav-links" role="menubar">
...
function template_button_strip($button_strip, $direction = '', $strip_options = array())
{
	...
	foreach ($button_strip as $key => $value)
	{
		if (!isset($value['test']) || !empty($context[$value['test']]))
			$buttons[] = '
								<li role="menuitem"><a' . (isset($value['id']) ? ' id="button_strip_' . $value['id'] . '"' : '') . ' class="button button_strip_' . $key . (!empty($value['active']) ? ' active' : '') . '" href="' . $value['url'] . '"' . (isset($value['custom']) ? ' ' . $value['custom'] : '') . '>' . $txt[$value['text']] . '</a></li>';
	}
